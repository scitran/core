...
class Core(base.RequestHandler):
        ...
        with open(filepath) as fileobj:
            while True:
                ...
                if not buf:
                    break
                if len(buf) < BUFSIZE:
                    # Add NULL padding to fill out the last block
                    remainder = BLOCKSIZE - (len(buf) % BLOCKSIZE)
                    buf += (b"\0" * remainder)
                yield buf
    ...
    def _archivestream(self, ticket):
        ...
        with tarfile.open(mode="w|", fileobj=stream) as tar:
            ...
            for filepath, arcpath, _ in targets:
                for block in self._add_file_to_tar_stream(tar, filepath, arcpath):
                    yield block

        # Capture blocks generated by closing the tar stream
        yield stream.getvalue()
